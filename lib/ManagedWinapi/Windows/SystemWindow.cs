...
namespace ManagedWinapi.Windows
{
    ...
    public class SystemWindow
    {
        ...
        public int SendGetMessage(uint message)
        {
            return SendGetMessage(message, 0);
        }

        public int SendGetMessage(uint message, uint param)
        {
            return SendMessage(new HandleRef(this, HWnd), message, new IntPtr(param), new IntPtr(0)).ToInt32();
        }

        public void SendSetMessage(uint message, uint value)
        {
            SendMessage(new HandleRef(this, HWnd), message, new IntPtr(value), new IntPtr(0));
        }

        public void SendSetMessage1(uint message, uint value, IntPtr lParam)
        {
            SendMessage(new HandleRef(this, HWnd), message, new IntPtr(value), lParam);
        }
